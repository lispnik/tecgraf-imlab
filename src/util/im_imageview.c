...
static void iViewUpdateSize(imImageView* image_view)
{
  ...
  case IM_VIEW_FIT:
    /* The image occupies all the cd_canvas and is centered, but does not changes its aspect ratio */
...
void imImageViewZoomDelta(imImageView* image_view, int delta, int x, int y)
{
  int dx, dy;

  /* dont work very well... */
  dx = image_view->Width / 2 - x;
  dy = image_view->Height / 2 - y;

  iViewMoveImage(image_view, dx, dy);
  ...
  image_view->ViewMode = IM_VIEW_NORMAL;
  image_view->ZoomFactor = ((double)ZoomTable1[image_view->ZoomIndex + IV_ZOOMZERO]) / ZoomTable2[image_view->ZoomIndex + IV_ZOOMZERO];
...
{
  *y = image_view->Height - *y -1;  /* Now we are in CD coordinates */
...
static int iViewMotion(Ihandle* self, int x, int y, char* r)
{
  ...
  if (isbutton1(r) && image_view->drag_x != 0xFFFF)
  {
...
static int iViewButton(Ihandle* self, int b, int pressed, int x, int y)
{
  imImageView* image_view = (imImageView*)IupGetAttribute(self, "imImageView");
  if (!image_view || !image_view->BitmapView->image || b != IUP_BUTTON1)
    return IUP_DEFAULT;
  ...
  if (imImageViewMouseCB)
  {
    int mode = pressed? IM_VIEW_MOUSEDOWN: IM_VIEW_MOUSEUP;
