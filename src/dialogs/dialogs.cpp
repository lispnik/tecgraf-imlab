...
#ifndef MOTIF
static void iDlgDrawImageFile(cdCanvas* cd_canvas, Ihandle* ih, const char* filename)
{
  int wc, hc;
  ...
  imImage* image = (imImage*)IupGetAttribute(ih, "_LASTIMAGE");
  char* last_file = IupGetAttribute(ih, "_LASTFILE");
  if (!last_file || !imStrEqual(last_file, filename))
  {
    int error;
    if (image) imImageDestroy(image);
    image = imFileImageLoadBitmap(filename, 0, &error);
    IupSetAttribute(ih, "_LASTIMAGE", (char*)image);
    IupStoreAttribute(ih, "_LASTFILE", (char*)filename);
