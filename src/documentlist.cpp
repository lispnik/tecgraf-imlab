...
imlabResultsWindow* imlabResultsWindowListSelect(const char *title)
{
  ...
  for (i = 0; i < results_window_list_count; i++)
  {
    ...
    char* res_title = IupGetAttribute(results_window->dialog, "TITLE");

    list_str[i] = (char*)malloc(strlen(res_title) + 10);
    sprintf(list_str[i], "[%d] %s", results_window->window_number, res_title);
  }
...
void imlabImageDocumentListRemove(imlabImageDocument* document)
{
  if (image_document_list_count > 1)
  {
    if (document == imlabGetCurrentDocument())
    {
