...
#define LENGTH_UNITS 12
  const char* length_str_utf8[LENGTH_UNITS] = { "m", "mm", "cm", "km", "nm", "\xC3\x85", "\xC2\xB5", "in", "ft", "yd", "mi", "NM" };
  const char* length_str_ansi[LENGTH_UNITS] = { "m", "mm", "cm", "km", "nm", "\xC5", "\xB5", "in", "ft", "yd", "mi", "NM" };
  const char* area_str_utf8[LENGTH_UNITS] = { "m\xC2\xB2", "mm\xC2\xB2", "cm\xC2\xB2", "km\xC2\xB2", "nm\xC2\xB2", "\xC3\x85\xC2\xB2", "\xC2\xB5\xC2\xB2", "sq in", "sq ft", "sq yd", "sq mi", "sq NM" };
  const char* area_str_ansi[LENGTH_UNITS] = { "m\xB2", "mm\xB2", "cm\xB2", "km\xB2", "nm\xB2", "\xC5\xB2", "\xB5\xB2", "sq in", "sq ft", "sq yd", "sq mi", "sq NM" };
  const char** length_str;
  const char** area_str;

  if (IupGetInt(NULL, "UTF8MODE"))
  {
    length_str = length_str_utf8;
    area_str = area_str_utf8;
  }
  else
  {
    length_str = length_str_ansi;
    area_str = area_str_ansi;
  }
  ...
  if (IupGetInt(NULL, "UTF8MODE"))
    strcat(area_units, "\xC2\xB2");
  else
    strcat(area_units, "\xB2");
  return area_units;
}
    ...
    else
    {
      ...
      if (result_type != IM_MEASURE_NONE)
      {
        ...
        else if (result_type == IM_MEASURE_AREA)
        {
          ...
          {
            if (IupGetInt(NULL, "UTF8MODE"))
              strcat(buffer, "px\xC2\xB2");
            else
              strcat(buffer, "px\xB2");
          }
          strcat(buffer, ")");
        }
      }
