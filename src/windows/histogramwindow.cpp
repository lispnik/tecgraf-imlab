...
static void iHistoUpdateMax(imlabHistogramWindow* histo_win)
{
  ...
  {
    for (h = 1; h < histo_win->num_histo; h++)
    {
  ...
  IupSetDouble(histo_win->plot, "AXS_YMAX", (double)max);
}
...
static int cmHistoMax(Ihandle* self, int c, char *after)
{
  ...
  if (new_max <= 0)
    ...
    IupSetDouble(histo_win->plot, "AXS_YMAX", (double)new_max);
...
static void iHistoCreateDialog(imlabHistogramWindow* histo_win)
{
  ...
  IupSetAttribute(plot, "MENUITEMVALUES", "Yes");   
  IupSetAttribute(plot, "MARGINBOTTOMAUTO", "No");   
  IupSetAttribute(plot, "MARGINBOTTOM", "80");
