...
static int color_saturation_preview(Ihandle* dialog, int param_index, void* user_data)
{
  ...
  if (imlabProcessPreviewCheckParam(dialog, param_index, "PARAM1", preview, &param_readonly))
  {
    ...
    double S_min = IupGetDouble((Ihandle*)IupGetAttribute(dialog, "PARAM0"), "VALUE");

    imProcessThresholdSaturation(image, NewImage, S_min);
...
static int color_saturation(Ihandle *parent)
{
  imlabImageDocument* document = imlabGetCurrentDocument();
  imImage *image = document->ImageFile->image;
  ...
  int show_preview = 0;
  static double S_min = 0;

  if (!IupGetParam(document->DlgTitle("Add Speckle Noise"), color_saturation_preview, preview,
                   "Percent: %R[0,1]\n"
                   "Preview: %b\n",
                   &S_min, &show_preview, NULL))
  {
  ...
  document->ChangeImage(NewImage, "ThresholdSaturation{S_min=%g}", S_min);
...
static int manual_threshold(Ihandle *parent)
{
  ...
  document->ChangeImage(NewImage, "Threshold{level=%g, value=1, negative=%d}", level, negative);
...
{
  imlabImageDocument* document1 = imlabGetCurrentDocument();
...
static void PlugInInit(Ihandle* mnProcess)
{
  ...
  submenu = IupMenu(
    ...
    imlabProcNewItem(mnProcess, "By Saturation...", "color_saturation", (Icallback)color_saturation, 1),
    NULL);
  ...
  IupSetAttribute((Ihandle*)IupGetAttribute(mnProcess, "color_saturation"), "imlabStatusHelp", "Threshold if saturation smaller than given.");
...
static void PlugInUpdate(Ihandle* mnProcess)
{
  ...
  imlabProcPlugInUpdateItem(mnProcess, "color_saturation", imImageIsByteRGB);
