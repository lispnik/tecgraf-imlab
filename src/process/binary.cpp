...
static int thin_nhmaps(Ihandle *parent)
{
  ...
  if (!imProcessBinThinNhMaps(image, NewImage))
    imImageDestroy(NewImage);
  else
    document->ChangeImage(NewImage, "BinThinNhMaps{}");

  return IUP_DEFAULT;
}

static int thin_zhangsuen(Ihandle *parent)
{
  imlabImageDocument* document = imlabGetCurrentDocument();
  imImage *image = document->ImageFile->image;
  ...
  if (!imProcessBinThinZhangSuen(image, NewImage))
    imImageDestroy(NewImage);
  else
    document->ChangeImage(NewImage, "BinThinZhangSuen{}");
...
static void PlugInInit(Ihandle* mnProcess)
{
  ...
  submenu = IupMenu(
    ...
    imlabProcNewItem(mnProcess, "Thin by N. Maps", "thin_nhmaps", (Icallback)thin_nhmaps, 1),
    imlabProcNewItem(mnProcess, "Thin by Zhang Suen", "thin_zhangsuen", (Icallback)thin_zhangsuen, 1),
    IupSeparator(),
...
//    imlabProcNewItem(mnProcess, "Watershed...", "watershed", (Icallback)watershed, 1), 
  ...
  IupSetAttribute((Ihandle*)IupGetAttribute(mnProcess, "thin_nhmaps"), "imlabStatusHelp", "Image Thinning using Neighborhood Maps from Graphics Gems IV.");
  IupSetAttribute((Ihandle*)IupGetAttribute(mnProcess, "thin_zhangsuen"), "imlabStatusHelp", "Image Thinning using Zhang Suen algorithm.");
...
{
  imlabProcPlugInUpdateItem(mnProcess, "thin_nhmaps", imImageIsBinary);
  imlabProcPlugInUpdateItem(mnProcess, "thin_zhangsuen", imImageIsBinary);
